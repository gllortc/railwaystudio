...
{
   public class ModelDAO : DataConsumer
   {
...
//      public override void Add(CollectionModel model)
//      {
...
//      /// </summary>
//      /// <param name="model">Una instáncia de RCModel que representa los datos del nuevo modelo.</param>
//      public override void Update(CollectionModel model)
//      {
...
//      public override int Delete(long modelId)
//      {
//         string sql = string.Empty;
...
//            ModelDigitalFunctionDAO dfmanager = new ModelDigitalFunctionDAO();
//            dfmanager.DeleteByModel(modelId);

//            // Remove maintenance information
//            ModelRevisionDAO revmanager = new ModelRevisionDAO();
...
//      }
      ...
      /// </remarks>
      public CollectionModel GetByID(long modelId)
      {
         string sql = string.Empty;
      ...
      /// Rellena un control ListView con una lista de modelos de la colección.
      /// </summary>
      public System.Data.DataTable FindByDigitalAddress(bool notInBlock)
      {
         string sql = string.Empty;
         ...
         try
         {
            ...
            if (!notInBlock)
            {
               sql = @"SELECT 
                          ...
                          End             As ""Block""
                       FROM 
                          " + ModelDAO.SQL_TABLE + @" m 
                          Left Join " + Layout.Persistence.ElementModelDAO.SQL_TABLE + @" em On (em.modelid = m.modid) 
                          Left Join " + Layout.Persistence.ElementDAO.SQL_TABLE + @"      e  On (e.id = em.elementid)
                       WHERE 
                          m.moddigitaladd > 0 
                       ORDER BY 
                          m.modname ASC";
            ...
            else
            {
               sql = @"SELECT 
                       ...
                       FROM 
                          ...
                          Left Join " + Layout.Persistence.ElementDAO.SQL_TABLE + @"      e  On (e.id = em.elementid)
                       WHERE 
                          m.moddigitaladd > 0 And 
                          m.modid Not In (SELECT DISTINCT modelid 
                                          FROM     " + Layout.Persistence.ElementModelDAO.SQL_TABLE + @" 
                                          ORDER BY modelid)  
                       ORDER BY 
                          m.modname ASC";
            }
      ...
      public CollectionModel ReadEntityRecord(SQLiteDataReader reader)
      {
         CollectionModel item = new CollectionModel();
         item.ID = reader.GetInt64(0);
         // item.AdminID = !reader.IsDBNull(1) ? reader.GetInt64(1) : 0;
         // item.Store = !reader.IsDBNull(2) ? reader.GetInt64(2) : 0;
         // item.CategoryID = !reader.IsDBNull(3) ? reader.GetInt64(3) : 0;
         ...
         // item.Manufacturer = !reader.IsDBNull(8) ? reader.GetInt64(8) : 0;
         // item.Scale = !reader.IsDBNull(9) ? reader.GetInt64(9) : 0;
         ...
         item.BuyIsPending = !reader.IsDBNull(13) ? reader.GetBoolean(13) : false;
         item.CouplersType = (CollectionModel.CouplerTypes)(!reader.IsDBNull(14) ? reader.GetInt32(14) : 0);
         item.HaveFunctionalPantos = !reader.IsDBNull(15) ? reader.GetBoolean(15) : false;
         item.HaveSound = !reader.IsDBNull(16) ? reader.GetBoolean(16) : false;
         item.IsLimited = !reader.IsDBNull(17) ? reader.GetBoolean(17) : false;
         ...
         item.LightFront = (CollectionModel.LightFrontType)(!reader.IsDBNull(22) ? reader.GetInt32(22) : 0);
         ...
         item.Units = !reader.IsDBNull(31) ? reader.GetInt32(31) : 0;
         item.DigitalAddress = !reader.IsDBNull(32) ? reader.GetInt32(32) : 0;
         // item.DigitalDecoder = !reader.IsDBNull(33) ? reader.GetInt64(33) : 0;
      ...
      public void AddAttachment(ModelFileAttachment attachment)
      {
         ...
         try
         {
            ...
            file.CopyTo(Path.Combine(Decoder.FilesPath, attachment.ModelID.ToString("00000") + "." + file.Name));
