...
{
   public class ModelDigitalFunctionDAO : CollectionDataEntity
   {
      ...
      public void Add(CollectionModel model)
      {
         ...
         try
         {
            foreach (ModelDigitalFunction func in model.DigitalFunctions)
            {
               ...
               SetParameter("funcid", func.FunctionId);
      ...
      /// </remarks>
      public List<ModelDigitalFunction> GetByModel(int modelId)
      {
         string sql = string.Empty;
         ...
         try
         {
            ...
            using (SQLiteDataReader read = ExecuteReader(sql))
            {
               while (read.Read())
               {
                  ...
                  item.FunctionId = read.GetInt32(0);
