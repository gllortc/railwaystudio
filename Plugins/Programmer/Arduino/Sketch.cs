...
namespace Rwm.Studio.Plugins.Designer.Arduino
{
   internal class Sketch
   {
      ...
      private const string PATH_SKETCH_FILENAME = Sketch.PATH_PROJECT_FOLDER + ".ino";
      ...
      {
         get { return Path.Combine(PluginData.PluginPath, Sketch.PATH_PROJECT_FOLDER); }
      }
      ...
      {
         get { return Path.Combine(PluginData.PluginPath, Sketch.PATH_PROJECT_FOLDER, Sketch.PATH_SKETCH_FILENAME); }
      }
      ...
      {
         get { return Path.Combine(PluginData.PluginPath, Sketch.PATH_PROJECT_FOLDER, Sketch.PATH_SKETCH_BUILD); }
      }
      ...
      private string LibraryPath
      {
         get
         {
            string libPath = Path.Combine(this.ProjectPath, "src", Sketch.LIBRARY_DCC_DECODER);
      ...
      private string BuildPath
      {
         get
         {
            string libPath = Path.Combine(PluginData.PluginPath, Sketch.PATH_BUILD);
            if (!Directory.Exists(libPath)) Directory.CreateDirectory(libPath);
            return libPath;
         }
      }
      ...
      private string CachePath
      {
         get
         {
            string libPath = Path.Combine(PluginData.PluginPath, Sketch.PATH_CACHE);
            if (!Directory.Exists(libPath)) Directory.CreateDirectory(libPath);
            return libPath;
         }
      }
      ...
      private void GenerateSketchSource()
      {
         ...
         sketch.AppendLine("#include \"src/" + Sketch.LIBRARY_DCC_DECODER + "/" + Sketch.LIBRARY_DCC_DECODER + ".h\"");
         ...
         File.WriteAllText(Path.Combine(this.ProjectPath, Sketch.PATH_SKETCH_FILENAME), sketch.ToString());
      }
      ...
      private void AddSketchLibraries()
      {
         ...
         source = source.Replace("#libfilename#", "\"" + Sketch.LIBRARY_DCC_DECODER + ".h\"");
         File.WriteAllText(Path.Combine(this.LibraryPath, Sketch.LIBRARY_DCC_DECODER + ".cpp"), source);

         File.WriteAllText(Path.Combine(this.LibraryPath, Sketch.LIBRARY_DCC_DECODER + ".h"), Properties.Resources.DCC_DECODER_H);
      }
