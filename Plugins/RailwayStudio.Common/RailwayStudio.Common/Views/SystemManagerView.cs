...
namespace Rwm.Studio.Plugins.Common.Views
{
   public partial class SystemManagerView : DevExpress.XtraEditors.XtraForm
   {
      ...
      {
         if (!(grdSystemsView.GetRow(e.RowHandle) is DigitalSystem system) || !system.ID.Equals(OTCContext.Project.DigitalSystem.ID))
         {
            StudioContext.UI.DrawRowIcon(DigitalSystem.SmallIcon, e);
         }
         else
         {
            StudioContext.UI.DrawRowIcon(Properties.Resources.ICO_SYSTEM_SELECT_16, e);
         }
      }
      ...
      private void CmdSystemSelect_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
      {
         ...
         try
         {
            ...
            DigitalSystem system = grdSystemsView.GetRow(selRows[0]) as DigitalSystem;
